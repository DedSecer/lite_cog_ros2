...
namespace lslidar_driver {
    ...
    bool LslidarDriver::loadParameters() {
        ...
        this->declare_parameter<bool>("filter_voxel", true);
        this->declare_parameter<double>("filter_leaf_size_m", 0.05);
        this->declare_parameter<bool>("filter_box", true);
        this->declare_parameter<bool>("crop_negative", true);
        this->declare_parameter<bool>("keep_organized", false); 
        this->declare_parameter<double>("filter_front_m", 0.15);
        this->declare_parameter<double>("filter_back_m", 0.6);
        this->declare_parameter<double>("filter_left_m", 0.25);
        this->declare_parameter<double>("filter_right_m", 0.25);
        this->declare_parameter<double>("filter_lower_height_m", 0.5);
        this->declare_parameter<double>("filter_higher_height_m", 0.1);
        this->declare_parameter<bool>("filter_plane", true);
        this->declare_parameter<double>("plane_a", 0.0);
        this->declare_parameter<double>("plane_b", 0.288232);
        this->declare_parameter<double>("plane_c", 0.95756);
        this->declare_parameter<double>("plane_d", 0.0);
        this->declare_parameter<double>("plane_distance_threshold", 0.05);
        this->declare_parameter<double>("plane_filter_x_min", -0.6);
        this->declare_parameter<double>("plane_filter_x_max", 0.6);
        this->declare_parameter<double>("plane_filter_y_min", 0.0);
        this->declare_parameter<double>("plane_filter_y_max", 1.7);
        ...
        this->get_parameter("filter_voxel", filter_voxel_);
        this->get_parameter("filter_leaf_size_m", filter_leaf_size_m_);

        this->get_parameter("filter_box", filter_box_);
        this->get_parameter("crop_negative", crop_negative_);

        this->get_parameter("keep_organized", keep_organized_); 
        this->get_parameter("filter_front_m", filter_front_m_);
        this->get_parameter("filter_back_m", filter_back_m_);
        this->get_parameter("filter_left_m", filter_left_m_);
        this->get_parameter("filter_right_m", filter_right_m_);
        this->get_parameter("filter_lower_height_m", filter_lower_height_m_);
        this->get_parameter("filter_higher_height_m", filter_higher_height_m_);

        this->get_parameter("filter_plane", filter_plane_);
        this->get_parameter("plane_a", plane_a_);
        this->get_parameter("plane_b", plane_b_);
        this->get_parameter("plane_c", plane_c_);
        this->get_parameter("plane_d", plane_d_);
        this->get_parameter("plane_distance_threshold", plane_distance_threshold_);
        plane_sqrt_dividend_ = sqrt(plane_a_ * plane_a_ + plane_b_ * plane_b_ + plane_c_ * plane_c_);
        this->get_parameter("plane_filter_x_min", plane_filter_x_min_);
        this->get_parameter("plane_filter_x_max", plane_filter_x_max_);
        this->get_parameter("plane_filter_y_min", plane_filter_y_min_);
        this->get_parameter("plane_filter_y_max", plane_filter_y_max_);

        RCLCPP_INFO_STREAM(get_logger(),
            "filter_vexel: " << filter_voxel_
        ...
        while (rclcpp::ok()) {
            ...
            if (rc == 0) {
               ...
               if (is_print_working_time) {
                    ...
                    RCLCPP_INFO(this->get_logger(),"total working time: %d hours:%d minutes", total_working_time / 60, total_working_time % 60);
                    RCLCPP_INFO(this->get_logger(),"less than     -40 degrees  working time: %d hours:%d minutes", less_minus40_degree_working_time / 60, less_minus40_degree_working_time % 60);
                    RCLCPP_INFO(this->get_logger(),"-40 degrees ~ -10 degrees  working time: %d hours:%d minutes", minus40_to_minus10_working_time / 60, minus40_to_minus10_working_time % 60);
                    RCLCPP_INFO(this->get_logger(),"-10 degrees ~ 30  degrees  working time: %d hours:%d minutes", minus10_to_30_working_time / 60, minus10_to_30_working_time % 60);
                    RCLCPP_INFO(this->get_logger(),"30  degrees ~ 70  degrees  working time: %d hours:%d minutes", positive30_to_70_working_time / 60, positive30_to_70_working_time % 60);
                    RCLCPP_INFO(this->get_logger(),"70  degrees ~ 100 degrees  working time: %d hours:%d minutes", positive70_to_100_working_time / 60, positive70_to_100_working_time % 60);
...
//        output_scan.scan_time = scan_time_;
    ...
    void LslidarDriver::publishPointcloud() {
        ...
        if (pcl_type) {
            ...
            if(filter_plane_){
                ...
                pc_msg.header.stamp = rclcpp::Clock(RCL_STEADY_TIME).now();
    ...
    bool LslidarDriver::poll() {
 	...
 	if(!start_process_msop_) return false;
        ...
        if (!is_get_scan_altitude_ && pkt.data[1211] == 0x46) {                           //old C32w
            RCLCPP_INFO(this->get_logger(),"byte[1211] == 0x46: old C32W");
            is_new_c32w_ = false;
            for (int k = 0; k < 32; ++k) {
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx) {
            if (!is_new_c32w_ && "c32_70" == c32_type) {
