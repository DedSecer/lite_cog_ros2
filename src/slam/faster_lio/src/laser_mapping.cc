...
bool LaserMapping::LoadParams() {
    ...
    declare_parameter<bool>("path_save_en", true);
    declare_parameter<bool>("publish.path_publish_en", true);
    ...
    declare_parameter<bool>("publish.scan_effect_pub_en", false);
    declare_parameter<std::string>("publish.tf_imu_frame", "body");
    declare_parameter<std::string>("publish.tf_world_frame", "camera_init");

    declare_parameter<int>("max_iteration", 4);
    declare_parameter<float>("esti_plane_threshold", 0.1);
    declare_parameter<std::string>("map_file_path", "");
    declare_parameter<bool>("common.time_sync_en", false);
    declare_parameter<double>("filter_size_surf", 0.5);
    declare_parameter<double>("filter_size_map", 0.0);
    declare_parameter<double>("cube_side_length", 200);
    declare_parameter<float>("mapping.det_range", 300.f);
    ...
    declare_parameter<double>("preprocess.blind", 0.01);
    declare_parameter<float>("preprocess.time_scale", 1e-3);
    declare_parameter<int>("preprocess.lidar_type", 1);
    declare_parameter<int>("preprocess.scan_line", 16);
    declare_parameter<int>("point_filter_num", 2);
    declare_parameter<bool>("feature_extract_enable", false);
    declare_parameter<bool>("runtime_pos_log_enable", true);
    ...
    declare_parameter<std::vector<double>>("mapping.extrinsic_T", std::vector<double>());
    declare_parameter<std::vector<double>>("mapping.extrinsic_R", std::vector<double>());

    declare_parameter<float>("ivox_grid_resolution", 0.2);
    declare_parameter<int>("ivox_nearby_type", 18);



    get_parameter<bool>("path_save_en", path_save_en_);
    get_parameter<bool>("publish.path_publish_en", path_pub_en_);
    get_parameter<bool>("publish.scan_publish_en", scan_pub_en_);
    get_parameter<bool>("publish.dense_publish_en", dense_pub_en_);
    get_parameter<bool>("publish.scan_bodyframe_pub_en", scan_body_pub_en_);
    get_parameter<bool>("publish.scan_effect_pub_en", scan_effect_pub_en_);
    get_parameter<std::string>("publish.tf_imu_frame", tf_imu_frame_);
    get_parameter<std::string>("publish.tf_world_frame", tf_world_frame_);

    get_parameter<int>("max_iteration", options::NUM_MAX_ITERATIONS);
    get_parameter<float>("esti_plane_threshold", options::ESTI_PLANE_THRESHOLD);
    get_parameter<std::string>("map_file_path", map_file_path_);
    get_parameter<bool>("common.time_sync_en", time_sync_en_);
    get_parameter<double>("filter_size_surf", filter_size_surf_min);
    get_parameter<double>("filter_size_map", filter_size_map_min_);
    get_parameter<double>("cube_side_length", cube_len_);
    get_parameter<float>("mapping.det_range", det_range_);
    ...
    get_parameter<double>("preprocess.blind", preprocess_->Blind());
    get_parameter<float>("preprocess.time_scale", preprocess_->TimeScale());
    get_parameter<int>("preprocess.lidar_type", lidar_type);
    get_parameter<int>("preprocess.scan_line", preprocess_->NumScans());
    get_parameter<int>("point_filter_num", preprocess_->PointFilterNum());
    get_parameter<bool>("feature_extract_enable", preprocess_->FeatureEnabled());
    get_parameter<bool>("runtime_pos_log_enable", runtime_pos_log_);
    get_parameter<bool>("mapping.extrinsic_est_en", extrinsic_est_en_);
    get_parameter<bool>("pcd_save.pcd_save_en", pcd_save_en_);
    get_parameter<int>("pcd_save.interval", pcd_save_interval_);
    get_parameter<std::vector<double>>("mapping.extrinsic_T", extrinT_);
    get_parameter<std::vector<double>>("mapping.extrinsic_R", extrinR_);

    get_parameter<float>("ivox_grid_resolution", ivox_options_.resolution_);
    get_parameter<int>("ivox_nearby_type", ivox_nearby_type);
...
void LaserMapping::SubAndPubToROS(){
    ...
    declare_parameter<std::string>("common.lid_topic", "/livox/lidar");
    get_parameter("common.lid_topic", lidar_topic);
    declare_parameter<std::string>("common.imu_topic", "/livox/imu");
    get_parameter("common.imu_topic", imu_topic);    

    if (preprocess_->GetLidarType() == LidarType::AVIA) {
        // sub_pcl_ = this->create_subscription<faster_lio_interface::msg::CustomMsg>(
        //     lidar_topic, 
        //     200000, 
        //     [this](const faster_lio_interface::msg::CustomMsg::SharedPtr msg){
...
LaserMapping::LaserMapping() : rclcpp::Node("laser_mapping"), br(*this){
...
void LaserMapping::StandardPCLCallBack(const sensor_msgs::msg::PointCloud2::SharedPtr msg){
    double t_diff = Stamp2Sec(msg->header.stamp) - last_timestamp_lidar_;
    if(t_diff > 0.2){
        LOG(INFO) << "Disgarding timeout frame, t_diff: " << t_diff;
        last_timestamp_lidar_ = Stamp2Sec(msg->header.stamp);
        return;
    }
    ...
    Timer::Evaluate(
        [&, this]() {
            scan_count_++;
            if (Stamp2Sec(msg->header.stamp) < last_timestamp_lidar_) {
            ...
            time_buffer_.push_back(Stamp2Sec(msg->header.stamp));
            last_timestamp_lidar_ = Stamp2Sec(msg->header.stamp);
        },
        "Preprocess (Standard)");
    mtx_buffer_.unlock();
...
void LaserMapping::LivoxPCLCallBack(const faster_lio_interface::msg::CustomMsg::SharedPtr msg){
    mtx_buffer_.lock();
    Timer::Evaluate(
        [&, this]() {
            scan_count_++;
...
void LaserMapping::IMUCallBack(const sensor_msgs::msg::Imu::SharedPtr msg_in) {
    ...
    if (abs(timediff_lidar_wrt_imu_) > 0.1 && time_sync_en_) {
        msg->header.stamp = rclcpp::Time(
            GetIntSec(timediff_lidar_wrt_imu_ + Stamp2Sec(msg_in->header.stamp)),
            GetIntNanosec(timediff_lidar_wrt_imu_ + Stamp2Sec(msg_in->header.stamp))
        );
    }
...
bool LaserMapping::SyncPackages() {
    ...
    double imu_time = Stamp2Sec(imu_buffer_.front()->header.stamp);
    measures_.imu_.clear();
    while ((!imu_buffer_.empty()) && (imu_time < lidar_end_time_)) {
        imu_time = Stamp2Sec(imu_buffer_.front()->header.stamp);
...
void LaserMapping::PublishPath() {
    ...
    msg_body_pose_.header.stamp = rclcpp::Time(
        GetIntSec(lidar_end_time_),
        GetIntNanosec(lidar_end_time_)
    );
...
void LaserMapping::PublishOdometry() {
    ...
    odom_aft_mapped_.header.stamp = rclcpp::Time(
        GetIntSec(lidar_end_time_),
        GetIntNanosec(lidar_end_time_)
    );  // ros::Time().fromSec(lidar_end_time_);
    ...
    transform.transform.rotation.w = odom_aft_mapped_.pose.pose.orientation.w;
    transform.transform.rotation.x = odom_aft_mapped_.pose.pose.orientation.x;
    transform.transform.rotation.y = odom_aft_mapped_.pose.pose.orientation.y;
    transform.transform.rotation.z = odom_aft_mapped_.pose.pose.orientation.z;
    ...
    if (pcd_save_en_) {
        *pcl_wait_save_ += *laserCloudWorld;
        ...
        if (pcl_wait_save_->size() > 0 && pcd_save_interval_ > 0 && scan_wait_num >= pcd_save_interval_) {
            ...
            //                            std::string(".pcd"));
            std::string all_points_dir(std::string("/home/ysc/lite_cog_ros2/system/map/lite3") + std::string(".pcd"));
...
void LaserMapping::Savetrajectory(const std::string &traj_file) {
    ...
    for (const auto &p : path_.poses) {
        ofs << std::fixed << std::setprecision(6) << Stamp2Sec(p.header.stamp) << " " << std::setprecision(15)
