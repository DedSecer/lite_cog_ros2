...
void ImuProcess::UndistortPcl(const common::MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state,
    ...
    for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++) {
        ...
        if (Stamp2Sec(tail->header.stamp) < last_lidar_end_time_) {
            continue;
        }
        ...
        if (Stamp2Sec(head->header.stamp) < last_lidar_end_time_) {
            dt = Stamp2Sec(tail->header.stamp) - last_lidar_end_time_;
        } else {
        ...
        double &&offs_t = Stamp2Sec(tail->header.stamp) - pcl_beg_time;
